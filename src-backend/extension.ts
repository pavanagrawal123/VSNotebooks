...
/**
     ...
     * If the custom path is set, the extension will check to see if there exists a jupyter 
     * installation. If there is not one it will fall back to not using a custom path.
        ...
        if (!JupyterManager.isJupyterInPath(JupyterManager.getScriptsLocation())) {
            ...
            if (!JupyterManager.isJupyterInPath()) {
                ...
                if (!JupyterManager.isJupyterInPath()) {
                    ...
                    let jupyterManager = new JupyterManager(true);

...
export function activate(context: vscode.ExtensionContext) {
     ...
     */
    const webview: WebviewController = new WebviewController(context);
     ...
     */
    const userInteraction: UserInteraction = new UserInteraction(context);
     ...
     */
    const cardManager: CardManager = new CardManager();
    ...
    webview.onOpenInBrowser((index) => {
        ...
        const cardId = cardManager.getCardId(index);
        const fileName = "exportedCardTmp_" + cardId + ".ipynb";
    ...
    userInteraction.onFullSetup(() => {
        if (!panelInitialised) {
            ...
            const choices = ["Create a new notebook", "Enter details manually"];
            const runningNotebooks = JupyterManager.getRunningNotebooks();
            ...
            vscode.window.showOpenDialog(options).then((fileUris) => {
                fileUris.map((fileUri) => {
                    vscode.window.showTextDocument(fileUri).then((textEditor) => {
                        const jsonContent: JSONObject = JSON.parse(textEditor.document.getText());
    ...
    vscode.window.onDidChangeActiveTextEditor((input) => {
        if (panelInitialised) {
            ...
            const kernel = UserInteraction.determineKernel();
     ...
     */
function initialiseJupyterLocally(initialisePanel:
    ({ baseUrl, token }: { baseUrl: any; token: any; }) => void, localJupyter: boolean) {
    if (JupyterManager.getScriptsLocationIfSpecified()) {
        if (!JupyterManager.isJupyterInPath(JupyterManager.getScriptsLocationIfSpecified())) {
            ...
            vscode.window.showErrorMessage(
                `VS Notebooks cannot find Jupyter in your custom environment or general path.
                Install using pip? Note: This defaults to the pip available on your path not environment.`, "Install")
                .then((data) => JupyterManager.installJupyter(data));
        } else {
            const jupyterManager = new JupyterManager();

...
export class UserInteraction {
     ...
     */
    public static askJupyterInfo(): Promise<{baseUrl: string, token: string}> {
     ...
     */
    public static savePdf(pdf: string) {
     ...
     */
    private newCard() {
        const trimNewlines = (s) => s.replace(/^(\r?\n)+|(\r?\n)+$/g, "");
        ...
        if (vscode.window.activeTextEditor.selection.isEmpty) {
            ...
            const lineNr = vscode.window.activeTextEditor.selection.start.line;
            const sourceCode = vscode.window.activeTextEditor.document.lineAt(lineNr).text;
            this._onNewCard.fire(trimNewlines(sourceCode));

            // Advance text cursor to the next line
            const newPos = new vscode.Position(lineNr + 1, 0);
            const newSelection = new vscode.Selection(newPos, newPos);
            vscode.window.activeTextEditor.selection = newSelection;
        } else {
            // Send selection to interpreter
            const sourceCode = vscode.window.activeTextEditor.document.getText(vscode.window.activeTextEditor.selection);
            this._onNewCard.fire(trimNewlines(sourceCode));
        }
    }
}
